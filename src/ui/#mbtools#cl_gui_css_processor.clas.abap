  ...
  PRIVATE SECTION.
    TYPES:
      BEGIN OF ty_css_var,
        ...
        value TYPE string,
      END OF ty_css_var,
      ty_css_var_tab TYPE SORTED TABLE OF ty_css_var WITH UNIQUE KEY name.
    ...
    METHODS:
      get_css_vars_in_string
        ...
        RETURNING
          VALUE(rt_variables) TYPE ty_css_var_tab,
      resolve_var_recursively
        IMPORTING
          iv_variable_name TYPE string
        CHANGING
          ct_variables     TYPE ty_css_var_tab
        RAISING
          /mbtools/cx_exception.
    DATA:
...
CLASS /MBTOOLS/CL_GUI_CSS_PROCESSOR IMPLEMENTATION.
  ...
  METHOD process.
    DATA:
      ...
      lt_css_variables    TYPE ty_css_var_tab,
      lt_css_vars_in_file TYPE ty_css_var_tab.
  ...
  METHOD resolve_var_recursively.
    ...
    IF sy-subrc = 0.
      ...
      ENDDO.
    ELSE.
      /mbtools/cx_exception=>raise( |CSS variable { iv_variable_name } not resolveable| ).
    ENDIF.
  ENDMETHOD.
ENDCLASS.
