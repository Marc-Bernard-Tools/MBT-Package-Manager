...
CLASS ltcl_test IMPLEMENTATION.
  ...
  METHOD can_create_expiring_log_days.
    ...
    CALL FUNCTION 'BAL_LOG_HDR_READ'
      EXPORTING
        i_log_handle = expiring_log->mv_handle
      IMPORTING
        e_s_log      = act_header.
  ...
  METHOD can_add_log_context.
    ...
    DATA: log                 TYPE REF TO /mbtools/if_logger,
    ...
    log = /mbtools/cl_logger=>new( iv_context = random_country_data ).

    CALL FUNCTION 'BAL_LOG_HDR_READ'
      EXPORTING
        i_log_handle = log->mv_handle
      IMPORTING
        e_s_log      = act_header.
  ...
  METHOD can_add_to_log.
    ...
    cl_aunit_assert=>assert_equals(
      exp = 'Itestthelogger.'
      act = get_first_message( anon_log->mv_handle )
  ...
  METHOD can_add_to_named_log.
    ...
    cl_aunit_assert=>assert_equals(
      ...
      act = get_first_message( named_log->mv_handle )
  ...
  METHOD auto_saves_named_log.
    ...
    APPEND named_log->mv_db_number TO log_numbers.
  ...
  METHOD auto_saves_reopened_log.
    ...
    APPEND reopened_log->mv_db_number TO log_numbers.
    CALL FUNCTION 'BAL_DB_LOAD'
      EXPORTING
        i_t_lognumber = log_numbers.

    get_messages( EXPORTING log_handle  = reopened_log->mv_handle
  ...
  METHOD can_log_bapirettab.
    ...
    get_messages( EXPORTING log_handle  = anon_log->mv_handle
                  IMPORTING texts       = act_texts
  ...
  METHOD can_log_chained_exceptions.
    ...
    cl_aunit_assert=>assert_equals(
      exp = format_message( id = 'SABP_UNIT' no = 000 v1 = 'This' v2 = 'is' v3 = 'test' v4 = 'message' )
      act = act_text
      msg = 'Did not log chained exception correctly' ).
  ENDMETHOD.
  ...
  METHOD can_log_any_deep_structure.
    ...
    exp_line = 'comp1 = Demo'.
    APPEND exp_line TO exp_table.
    exp_line = 'deep-comp1 = Inner component'.
    APPEND exp_line TO exp_table.
    exp_line = 'deep-comp2 = 10'.
    APPEND exp_line TO exp_table.
    exp_line = '--- End of structure ---'.
    APPEND exp_line TO exp_table.
  ...
  METHOD can_add_msg_context.
    ...
    anon_log->add( iv_obj_to_log = 'Here is some text'
                   iv_context = addl_context ).
  ...
  METHOD can_add_callback_sub.
    ...
    anon_log->add( iv_obj_to_log = 'Message with Callback'
                   iv_callback_form = 'FORM'
                   iv_callback_prog = 'PROGRAM' ).
  ...
  METHOD must_use_factory.
    ...
    TRY.
        CREATE OBJECT log TYPE ('/MBTOOLS/CL_LOGGER').
